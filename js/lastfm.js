        ...
        if (!data.error) {
            if (data.topartists.artist.length === 0) {
                ...
                for (var i in data.topartists.artist) {
                    ...
                    listData += '<li class="lastfm-top-artists__item"><a class="lastfm-top-artists__link" href="' + item.url + '"><img class="img-circle lastfm__img" height="174" src="' + imgUrl + '" width="174" /><div class="lastfm-top-artists__name img-circle"><p><strong>' + item.name + '</strong><br />' + item.playcount + ' plays</p></div></a></li>';
                }
